#!/usr/bin/env python3
"""Migrate data from MySQL `qitech.dmd_unit_of_measures` to Postgres `public.dmd_lookup_units_of_measure`.
...
def fetch_mysql_rows(connection, offset: int, limit: int) -> List[Dict[str, Any]]:
    with connection.cursor() as cur:
        cur.execute(
            "SELECT cd AS id, `desc` AS description, created_at, updated_at FROM dmd_unit_of_measures ORDER BY cd LIMIT %s OFFSET %s",
            (limit, offset),
        )
        return cur.fetchall()
...
def get_existing_ids(pg_conn) -> set:
    with pg_conn.cursor() as cur:
        cur.execute("SELECT id FROM public.dmd_lookup_units_of_measure")
...
def insert_batch(pg_conn, rows: List[Dict[str, Any]]):
    ...
    sql = (
        "INSERT INTO public.dmd_lookup_units_of_measure (" + 
        ",".join(cols) + ") VALUES %s"
    )
