...
ROWS = [
    ("id", "int8 NOT NULL", "`cd`", "Uses the source route code from MySQL `dmd_routes` as the target Postgres ID."),
...
def make_doc(out_path: str):
    doc = Document()
    doc.add_heading('Migration mapping: MySQL qitech.dmd_routes -> Postgres public.dmd_lookup_routes', level=1)
    ...
    hdr_cells[2].text = 'Source (MySQL qitech.dmd_routes)'
