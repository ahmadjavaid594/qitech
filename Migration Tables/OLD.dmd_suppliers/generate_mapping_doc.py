...
ROWS = [
    ("id", "text NOT NULL", "`cd`", "Uses the source supplier code from MySQL `dmd_suppliers` as the target Postgres ID."),
...
def make_doc(out_path: str):
    doc = Document()
    doc.add_heading('Migration mapping: MySQL qitech.dmd_suppliers -> Postgres public.dmd_lookup_suppliers', level=1)
    ...
    hdr_cells[2].text = 'Source (MySQL qitech.dmd_suppliers)'
    ...
    doc.add_paragraph(' - The script uses `cd` from MySQL as the Postgres target `id` value.')
