#!/usr/bin/env python3
"""Migrate data from MySQL `qitech.dmd_suppliers` to Postgres `public.dmd_lookup_suppliers`.
...
def fetch_mysql_rows(connection, offset: int, limit: int) -> List[Dict[str, Any]]:
    with connection.cursor() as cur:
        cur.execute(
            "SELECT cd AS id, `desc` AS description, created_at, updated_at FROM dmd_suppliers ORDER BY cd LIMIT %s OFFSET %s",
            (limit, offset),
        )
        return cur.fetchall()
...
def get_existing_ids(pg_conn) -> set:
    with pg_conn.cursor() as cur:
        cur.execute("SELECT id FROM public.dmd_lookup_suppliers")
