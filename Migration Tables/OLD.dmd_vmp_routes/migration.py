#!/usr/bin/env python3
"""Migrate data from MySQL `qitech.dmd_vmp_routes` to Postgres `public.dmd_virtual_medicinal_product_routes`.
...
def fetch_mysql_rows(connection, offset: int, limit: int) -> List[Dict[str, Any]]:
    with connection.cursor() as cur:
        cur.execute(
            "SELECT vpid, route_cd, created_at, updated_at FROM dmd_vmp_routes ORDER BY id LIMIT %s OFFSET %s",
            (limit, offset),
        )
        return cur.fetchall()
...
def get_existing_route_pairs(pg_conn) -> set:
    with pg_conn.cursor() as cur:
        cur.execute(
            "SELECT virtual_medicinal_product_id, route_code FROM public.dmd_virtual_medicinal_product_routes"
        )
...
def transform_row(row: Dict[str, Any], virtual_product_map: Dict[int, str]) -> Dict[str, Any]:
    ...
    external_id = parse_int(source_vpid)
    if external_id is None:
        raise ValueError(f"Missing or invalid vpid value for source row: {source_vpid!r}")

    virtual_medicinal_product_id = virtual_product_map.get(external_id)
    if virtual_medicinal_product_id is None:
        raise ValueError(
            f"No target virtual_medicinal_product_id found for vpid external_id {external_id!r}"
        )

    route_code = parse_int(row.get("route_cd"))
    if route_code is None:
        raise ValueError(f"Missing or invalid route_cd value for route_code: {row.get('route_cd')!r}")
...
def insert_batch(pg_conn, rows: List[Dict[str, Any]]):
    ...
    sql = (
        "INSERT INTO public.dmd_virtual_medicinal_product_routes (" +
        ",".join(cols) + ") VALUES %s ON CONFLICT (virtual_medicinal_product_id, route_code) DO NOTHING"
    )
...
def main(dry_run: bool = False):
    ...
    try:
        ...
        existing_pairs = get_existing_route_pairs(pg_conn)
        logging.info("Found %d existing route pairs in Postgres", len(existing_pairs))
        ...
        while True:
            ...
            for r in rows:
                ...
                pair = (t["virtual_medicinal_product_id"], t["route_code"])
