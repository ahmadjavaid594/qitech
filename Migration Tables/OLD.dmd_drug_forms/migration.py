#!/usr/bin/env python3
"""Migrate data from MySQL `qitech.dmd_drug_forms` to Postgres `public.dmd_lookup_forms`.
...
def fetch_mysql_rows(connection, offset: int, limit: int) -> List[Dict[str, Any]]:
    with connection.cursor() as cur:
        cur.execute(
            "SELECT `cd`, `desc`, created_at, updated_at FROM dmd_drug_forms ORDER BY cd LIMIT %s OFFSET %s",
            (limit, offset),
        )
        return cur.fetchall()
...
def get_existing_ids(pg_conn) -> set:
    with pg_conn.cursor() as cur:
        cur.execute("SELECT id FROM public.dmd_lookup_forms")
...
def transform_row(row: Dict[str, Any]) -> Dict[str, Any]:
    cd = row.get("cd")
    if not cd:
        raise ValueError(f"Missing or invalid cd value: {cd!r}")
    
    row_id = parse_int(cd)
    if row_id is None:
        raise ValueError(f"cd value is not a valid integer: {cd!r}")
...
def insert_batch(pg_conn, rows: List[Dict[str, Any]]):
    if not rows:
        return
    cols = ["id", "description", "created_at", "updated_at"]
    ...
    sql = (
        "INSERT INTO public.dmd_lookup_forms (" +
        ",".join(cols) + ") VALUES %s ON CONFLICT (id) DO NOTHING"
    )
