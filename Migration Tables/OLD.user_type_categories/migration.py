#!/usr/bin/env python3
"""Migrate data from MySQL `qitech.user_type_categories` to Postgres `public.user_types`.
...
HEAD_OFFICE_OVERRIDES = {
    36: "d88a69fc-673e-4a98-890a-2660f80397be",  # Qitech
}
...
def fetch_mysql_rows(connection, offset: int, limit: int) -> List[Dict[str, Any]]:
    with connection.cursor() as cur:
        cur.execute(
            "SELECT id, name, head_office_id, created_at, updated_at FROM user_type_categories ORDER BY id LIMIT %s OFFSET %s",
            (limit, offset),
        )
        return cur.fetchall()


def build_company_maps(pg_conn) -> (Dict[int, str], Dict[str, str]):
    # Build two maps:
...
def insert_batch(pg_conn, rows: List[Dict[str, Any]]):
    ...
    sql = (
        ...
        ",".join(cols) + ") VALUES %s ON CONFLICT (company_id, \"name\") DO UPDATE SET external_id = COALESCE(user_types.external_id, EXCLUDED.external_id)"
    )
