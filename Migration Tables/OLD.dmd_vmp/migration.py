#!/usr/bin/env python3
"""Migrate data from MySQL `qitech.dmd_vmps` to Postgres `public.dmd_virtual_medicinal_products`.
...
def fetch_mysql_rows(connection, offset: int, limit: int) -> List[Dict[str, Any]]:
    with connection.cursor() as cur:
        cur.execute(
            "SELECT vpid, vtmid, name, basis_cd, pres_stat_cd, udfs, udfs_uomcd, created_at, updated_at FROM dmd_vmps ORDER BY vpid LIMIT %s OFFSET %s",
            (limit, offset),
        )
        return cur.fetchall()
...
def get_existing_external_ids(pg_conn) -> set:
    with pg_conn.cursor() as cur:
        cur.execute("SELECT external_id FROM public.dmd_virtual_medicinal_products WHERE external_id IS NOT NULL")
...
def transform_row(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    virtual_therapeutic_moiety_id = parse_int(row.get("vtmid"))
    basis_name_code = parse_smallint(row.get("basis_cd"))
    pres_status_code = parse_smallint(row.get("pres_stat_cd"))
    udfs = parse_numeric(row.get("udfs"))
    udfs_uom_code = parse_int(row.get("udfs_uomcd"))
    ...
    return {
        ...
        "invalid": False,
        "virtual_therapeutic_moiety_id": virtual_therapeutic_moiety_id,
        "name": row.get("name"),
        "basis_name_code": basis_name_code,
        "pres_status_code": pres_status_code,
        "udfs": udfs,
        "udfs_uom_code": udfs_uom_code,
...
def insert_batch(pg_conn, rows: List[Dict[str, Any]]):
    ...
    values = [
        (
            ...
            r["release_version"],
            r["invalid"],
            r["virtual_therapeutic_moiety_id"],
            r["name"],
            r["basis_name_code"],
            r["pres_status_code"],
            r["udfs"],
    ...
    sql = (
        "INSERT INTO public.dmd_virtual_medicinal_products (" +
        ",".join(cols) + ") VALUES %s"
    )
