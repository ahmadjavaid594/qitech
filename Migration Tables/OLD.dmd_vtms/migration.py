#!/usr/bin/env python3
"""Migrate data from MySQL `qitech.dmd_vtms` to Postgres `public.dmd_virtual_therapeutic_moieties`.
...
def fetch_mysql_rows(connection, offset: int, limit: int) -> List[Dict[str, Any]]:
    with connection.cursor() as cur:
        cur.execute(
            "SELECT vtmid, name, created_at, updated_at FROM dmd_vtms ORDER BY vtmid LIMIT %s OFFSET %s",
            (limit, offset),
        )
        return cur.fetchall()
...
def get_existing_external_ids(pg_conn) -> set:
    with pg_conn.cursor() as cur:
        cur.execute("SELECT external_id FROM public.dmd_virtual_therapeutic_moieties WHERE external_id IS NOT NULL")
...
def transform_row(row: Dict[str, Any]) -> Dict[str, Any]:
    external_id = parse_int(row.get("vtmid"))
    if external_id is None:
        raise ValueError(f"Missing or invalid vtmid value for external_id: {row.get('vtmid')!r}")
...
def insert_batch(pg_conn, rows: List[Dict[str, Any]]):
    ...
    sql = (
        "INSERT INTO public.dmd_virtual_therapeutic_moieties (" +
        ",".join(cols) + ") VALUES %s"
    )
