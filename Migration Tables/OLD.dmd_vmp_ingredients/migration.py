#!/usr/bin/env python3
"""Migrate data from MySQL `qitech.dmd_vmp_ingredients` to Postgres `public.dmd_virtual_medicinal_product_ingredients`.
...
def fetch_mysql_rows(connection, offset: int, limit: int) -> List[Dict[str, Any]]:
    with connection.cursor() as cur:
        cur.execute(
            "SELECT vpid, isid, basis_strength_cd, strength_value, strength_uomcd, created_at FROM dmd_vmp_ingredients ORDER BY id LIMIT %s OFFSET %s",
            (limit, offset),
        )
        return cur.fetchall()
...
def get_existing_ingredient_pairs(pg_conn) -> set:
    with pg_conn.cursor() as cur:
        cur.execute(
            "SELECT virtual_medicinal_product_id, ingredient_substance_id FROM public.dmd_virtual_medicinal_product_ingredients"
        )
...
def transform_row(row: Dict[str, Any], virtual_product_map: Dict[int, str]) -> Dict[str, Any]:
    ...
    ingredient_substance_id = parse_int(row.get("isid"))
    if ingredient_substance_id is None:
        raise ValueError(f"Missing or invalid isid value for ingredient_substance_id: {row.get('isid')!r}")

    basis_strength_code = parse_int(row.get("basis_strength_cd"))
    strength_num_val = parse_numeric(row.get("strength_value"))
    strength_num_uom = parse_int(row.get("strength_uomcd"))

    created_at = row.get("created_at") or datetime.now(timezone.utc)

    return {
        "virtual_medicinal_product_id": virtual_medicinal_product_id,
        "ingredient_substance_id": ingredient_substance_id,
        "basis_strength_code": basis_strength_code,
        "strength_num_val": strength_num_val,
        "strength_num_uom": strength_num_uom,
        "strength_denom_val": None,
        "strength_denom_uom": None,
...
def insert_batch(pg_conn, rows: List[Dict[str, Any]]):
    ...
    values = [
        (
            ...
            r["ingredient_substance_id"],
            r["basis_strength_code"],
            r["strength_num_val"],
            r["strength_num_uom"],
            r["strength_denom_val"],
            r["strength_denom_uom"],
            r["created_at"],
        )
        for r in rows
    ]
    ...
    sql = (
        "INSERT INTO public.dmd_virtual_medicinal_product_ingredients (" +
        ",".join(cols) + ") VALUES %s ON CONFLICT (virtual_medicinal_product_id, ingredient_substance_id) DO NOTHING"
    )
...
def main(dry_run: bool = False):
    ...
    try:
        ...
        existing_pairs = get_existing_ingredient_pairs(pg_conn)
        logging.info("Found %d existing ingredient pairs in Postgres", len(existing_pairs))
        ...
        while True:
            ...
            for r in rows:
                ...
                pair = (t["virtual_medicinal_product_id"], t["ingredient_substance_id"])
